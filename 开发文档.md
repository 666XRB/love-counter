这是一个非常私人且充满创意的点子。将 Github Pages 用于这类记录非常方便，因为它免费且稳定。

为了实现你的需求，尤其是**“不同设备同步数据”**这一核心难点，由于 Github Pages 只是静态托管（没有后端数据库），我们需要引入 **Supabase** 或 **LeanCloud** 这样的免费云数据库服务，或者利用 **WebDAV** 挂载坚果云等。

考虑到开发难度和 UI 美感，我建议使用 **Next.js (或 Vite + React) + Tailwind CSS + Lucide Icons**。

---

## 🛠 开发文档：爱计数 (Love Counter)

### 1. 技术栈选择

* **前端框架**: React.js (Vite 构建，轻量高效)
* **样式库**: Tailwind CSS + Shadcn UI (模拟 Windows 11 的亚克力/云母质感)
* **数据同步**: Supabase (免费层级足够，提供实时数据库，解决多设备同步问题)
* **导出功能**: jsPDF + html2canvas
* **部署**: GitHub Pages

### 2. 核心功能设计

* **热力图日历**: 仿 GitHub 贡献图，性爱频率越高，颜色越深（从浅粉到深紫）。
* **极简弹窗**: 点击日期即可快速记录（时间、持续时间、五星满意度）。
* **数据看板**: 展示“年度总次数”、“本月频率”、“最高纪录”等统计数据。
* **同步机制**: 使用统一的查询参数或简单的“房间号/密码”登录来同步 Supabase 里的数据。

---

## 💻 核心代码实现

### 第一步：数据库结构 (Supabase)

在 Supabase 中创建一个 `records` 表：

* `id`: uuid
* `created_at`: timestamp
* `rating`: integer (1-5)
* `note`: text
* `user_id`: string (用于你和你女朋友共享的唯一标识)

### 第二步：核心逻辑组件 (React + Tailwind)

```jsx
import React, { useState, useEffect } from 'react';
import { Calendar } from '@/components/ui/calendar'; // 使用 shadcn 组件
import { createClient } from '@supabase/supabase-js';
import { LineChart, Line, XAxis, Tooltip, ResponsiveContainer } from 'recharts';
import { Heart, Download, Share2 } from 'lucide-react';

const supabase = createClient('YOUR_SUPABASE_URL', 'YOUR_SUPABASE_ANON_KEY');

export default function LoveCounter() {
  const [date, setDate] = useState(new Date());
  const [records, setRecords] = useState([]);

  // 获取数据
  useEffect(() => {
    fetchRecords();
  }, []);

  const fetchRecords = async () => {
    const { data } = await supabase.from('records').select('*');
    setRecords(data);
  };

  // 记录一次爱爱
  const addRecord = async (rating) => {
    const newRecord = {
      date: date.toISOString(),
      rating: rating,
      pair_id: 'our-secret-id' // 你们共同的ID
    };
    await supabase.from('records').insert([newRecord]);
    fetchRecords();
  };

  // 计算颜色深度逻辑
  const getDayColor = (day) => {
    const count = records.filter(r => isSameDay(r.date, day)).length;
    if (count === 0) return 'bg-slate-100';
    if (count === 1) return 'bg-pink-200';
    if (count === 2) return 'bg-pink-400';
    return 'bg-pink-600'; // 次数越多颜色越深
  };

  return (
    <div className="min-h-screen bg-[#F3F3F3] p-4 md:p-8 font-sans text-slate-800">
      {/* Windows 11 风格卡片 */}
      <div className="max-w-4xl mx-auto bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-white p-6">
        <header className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Heart className="text-pink-500 fill-pink-500" /> 爱计数
          </h1>
          <button className="p-2 hover:bg-slate-100 rounded-full transition" onClick={exportPDF}>
            <Download size={20} />
          </button>
        </header>

        <div className="grid md:grid-cols-2 gap-8">
          {/* 日历部分 */}
          <section>
            <Calendar
              mode="single"
              selected={date}
              onSelect={setDate}
              className="rounded-md border shadow-sm"
            />
          </section>

          {/* 统计部分 */}
          <section className="space-y-6">
            <div className="bg-gradient-to-br from-pink-50 to-orange-50 p-6 rounded-xl border border-pink-100">
              <p className="text-sm text-slate-500">本月甜蜜次数</p>
              <h2 className="text-4xl font-black text-pink-600">24 次</h2>
            </div>
            
            {/* 快速打分记录 */}
            <div className="space-y-3">
              <p className="font-medium">记录今天的亲密瞬间：</p>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map(num => (
                  <button 
                    key={num}
                    onClick={() => addRecord(num)}
                    className="flex-1 py-2 rounded-lg bg-white border border-slate-200 hover:border-pink-400 hover:text-pink-500 transition"
                  >
                    {num} ⭐
                  </button>
                ))}
              </div>
            </div>
          </section>
        </div>
      </div>
    </div>
  );
}

```

---

## 🎨 UI & 交互细节说明

1. **Windows 11 视觉语言**:
* **背景**: 使用淡灰色（#F3F3F3）或微软的 Mica 材质背景图。
* **圆角**: 容器使用 `rounded-2xl` (16px)，小按钮使用 `rounded-lg` (8px)。
* **模糊效果**: 使用 CSS `backdrop-filter: blur(20px);` 实现毛玻璃质感，这会让页面看起来非常高级。


2. **数据同步原理**:
* 既然是静态页面，你可以给女朋友发送一个带参数的链接，例如 `https://yourname.github.io/love-counter?pair_id=xxxx`。
* 代码逻辑会自动识别 `pair_id`，从数据库调取属于你们两人的共同记录。


3. **PDF 报告**:
* 使用 `jsPDF` 捕捉页面中隐藏的一个“报告模版” div，生成包含：**本月趋势图表**、**评价分布柱状图**、**最频繁时间段**等数据的精美 PDF。



---

既然你提供了 LeanCloud 的凭证，我们可以直接使用 **LeanCloud 存储 SDK** 来处理数据。这对于静态页面（GitHub Pages）来说是完美的方案，因为它不需要你维护服务器，且在中国访问速度非常快。

以下是“爱计数 (Love Counter)”项目的完整开发细节、核心代码以及 Windows 11 风格的实现方案。

---

### 一、 项目结构与开发环境

1. **技术选型**:
* **框架**: React (使用 Vite 快速构建)
* **样式**: Tailwind CSS (用于实现高级 UI)
* **数据库**: LeanCloud (结构化数据存储)
* **报表导出**: `html2canvas` + `jspdf`


2. **安装依赖**:
```bash
npm install leancloud-storage lucide-react html2canvas jspdf date-fns

```



---

### 二、 核心数据库配置 (`src/db.js`)

我们将数据存储在一个名为 `Heartbeat` 的 Class 中。

```javascript
import AV from 'leancloud-storage';

AV.init({
  appId: "fXnYOxrBJqYINPMyp49sG3Sh-gzGzoHsz",
  appKey: "4VGgTvlU2vWsdjK4QxM9ArcB",
  serverURL: "https://fxnyoxrb.lc-cn-n1-shared.com"
});

export const LoveRecord = AV.Object.extend('LoveRecord');
export default AV;

```

---

### 三、 核心逻辑与 UI 实现 (`App.jsx`)

这段代码集成了 **Windows 11 亚克力质感**、**日历热力图逻辑**、**数据同步**以及**导出功能**。

```jsx
import React, { useState, useEffect, useRef } from 'react';
import AV, { LoveRecord } from './db';
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay } from 'date-fns';
import { Heart, Download, Plus, Star, BarChart3 } from 'lucide-react';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';

export default function App() {
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const reportRef = useRef();

  // 1. 初始化并拉取云端数据 (实时同步)
  useEffect(() => {
    fetchData();
  }, []);

  const fetchData = async () => {
    const query = new AV.Query('LoveRecord');
    query.limit(1000); // 获取最近记录
    const results = await query.find();
    setRecords(results.map(r => ({
      id: r.id,
      date: new Date(r.get('date')),
      rating: r.get('rating'),
      note: r.get('note')
    })));
    setLoading(false);
  };

  // 2. 核心功能：添加记录
  const addLoveRecord = async (rating) => {
    const record = new LoveRecord();
    record.set('date', new Date()); // 当前时间
    record.set('rating', rating);
    await record.save();
    fetchData(); // 重新加载实现同步
    alert('记录成功！❤️');
  };

  // 3. 计算日历颜色深度 (Windows 风格热力图)
  const getIntensityClass = (day) => {
    const count = records.filter(r => isSameDay(r.date, day)).length;
    if (count === 0) return 'bg-white/40'; 
    if (count === 1) return 'bg-pink-200 border-pink-300';
    if (count === 2) return 'bg-pink-400 border-pink-500 text-white';
    return 'bg-pink-600 border-pink-700 text-white shadow-lg shadow-pink-200';
  };

  // 4. 导出 PDF 报表
  const exportPDF = () => {
    const input = reportRef.current;
    html2canvas(input).then((canvas) => {
      const imgData = canvas.toDataURL('image/png');
      const pdf = new jsPDF('p', 'mm', 'a4');
      pdf.addImage(imgData, 'PNG', 10, 10, 190, 0);
      pdf.save("爱计数-年度报告.pdf");
    });
  };

  return (
    <div className="min-h-screen bg-[#f0f2f5] text-slate-800 p-4 md:p-10 font-sans selection:bg-pink-200">
      {/* 磨砂玻璃容器 */}
      <div ref={reportRef} className="max-w-5xl mx-auto bg-white/70 backdrop-blur-xl rounded-[24px] shadow-2xl border border-white/50 overflow-hidden">
        
        {/* Header - Windows 11 Style */}
        <nav className="p-6 border-b border-gray-200/50 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2 tracking-tight">
              <Heart className="fill-pink-500 text-pink-500 animate-pulse" /> 
              HYJ❤️XRBの爱计数 <span className="text-sm font-normal text-slate-400">v1.0</span>
            </h1>
          </div>
          <button onClick={exportPDF} className="flex items-center gap-2 px-4 py-2 bg-white/80 hover:bg-pink-50 rounded-full border border-gray-200 transition-all active:scale-95 shadow-sm">
            <Download size={18} /> 导出 PDF 报告
          </button>
        </nav>

        <div className="grid lg:grid-cols-3 gap-0">
          {/* 左侧：日历面板 */}
          <div className="lg:col-span-2 p-8 border-r border-gray-100">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold">年度甜蜜图谱</h2>
              <div className="flex gap-2 text-xs items-center text-slate-400">
                Less <div className="w-3 h-3 bg-white/40 border"></div>
                <div className="w-3 h-3 bg-pink-200"></div>
                <div className="w-3 h-3 bg-pink-400"></div>
                <div className="w-3 h-3 bg-pink-600"></div> More
              </div>
            </div>

            {/* 模拟热力图日历 */}
            <div className="grid grid-cols-7 gap-2">
              {eachDayOfInterval({
                start: startOfMonth(new Date()),
                end: endOfMonth(new Date())
              }).map(day => (
                <div key={day.toString()} className={`aspect-square rounded-lg border flex flex-col items-center justify-center transition-all ${getIntensityClass(day)}`}>
                  <span className="text-xs opacity-60">{format(day, 'd')}</span>
                  {records.filter(r => isSameDay(r.date, day)).length > 0 && <Heart size={10} className="mt-1 fill-current"/>}
                </div>
              ))}
            </div>
          </div>

          {/* 右侧：操作与统计 */}
          <div className="p-8 bg-gray-50/30 space-y-8">
            <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              <h3 className="text-sm font-medium text-slate-500 mb-4 flex items-center gap-2">
                <Plus size={16}/> 快速记录
              </h3>
              <div className="flex flex-wrap gap-3">
                {[1, 2, 3, 4, 5].map(score => (
                  <button 
                    key={score}
                    onClick={() => addLoveRecord(score)}
                    className="w-10 h-10 rounded-full border border-pink-100 flex items-center justify-center hover:bg-pink-500 hover:text-white transition-all hover:scale-110 active:scale-90"
                  >
                    {score}
                  </button>
                ))}
              </div>
              <p className="mt-4 text-[10px] text-slate-400 text-center italic">点击分值（1-5星）立即存入云端</p>
            </section>

            <section className="space-y-4">
              <h3 className="text-sm font-medium text-slate-500 flex items-center gap-2">
                <BarChart3 size={16}/> 数据洞察
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-xl border border-gray-100 text-center">
                  <div className="text-2xl font-bold text-pink-500">{records.length}</div>
                  <div className="text-[10px] uppercase tracking-wider text-slate-400">累计次数</div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-gray-100 text-center">
                  <div className="text-2xl font-bold text-orange-400">
                    {(records.reduce((acc, curr) => acc + curr.rating, 0) / (records.length || 1)).toFixed(1)}
                  </div>
                  <div className="text-[10px] uppercase tracking-wider text-slate-400">平均指数</div>
                </div>
              </div>
            </section>
          </div>
        </div>
      </div>
    </div>
  );
}

```

---

### 四、 开发与部署细节说明

#### 1. 数据同步（如何让你女朋友看到一样的）

因为你在代码中配置了相同的 `AppID` 和 `AppKey`，LeanCloud 会指向同一个数据库。

* **同步逻辑**：每当你点击记录，数据会上传到云端。你女朋友刷新页面（或者在代码中加入 `setInterval` 定时轮询），就会调用 `fetchData()` 获取最新的数据。
* **多设备支持**：该 UI 采用 Tailwind 响应式设计，在手机上会自动变为单列布局，方便随时随地在床头记录。

#### 2. UI 高级感处理

* **亚克力效果**：使用了 `backdrop-blur-xl` 和 `bg-white/70`。这会让背景透出一种朦胧感，非常符合 Windows 11 的视觉标准。
* **颜色阶梯**：日历格子会根据当天 `records.length` 的多少改变颜色，次数越多，粉色越深。

#### 3. 部署到 GitHub Pages

1. 在 `package.json` 中添加 `"homepage": "https://<your-username>.github.io/love-counter"`。
2. 运行命令：
```bash
npm run build
npm run deploy

```


(需要先安装 `gh-pages` 插件)。

#### 4. 安全建议 (重要)

因为你是静态部署，MasterKey **绝对不要** 写在前端代码里。上面的代码只使用了 `AppKey`，这在 LeanCloud 中是安全的（受 ACL 限制）。建议你在 LeanCloud 控制台开启 **“仅创建者可写”** 或者简单的权限设置，防止外人恶意注入数据。

为了确保你能够从零开始、直接通过 GitHub Pages 成功部署并让两人同步使用，我们需要在细节上做到“开箱即用”。

以下是为你完善的**保姆级开发文档**，包含项目初始化、环境避坑指南以及增强版的导出与 UI 逻辑。

---

## 📖 “爱计数”项目完整开发文档

### 1. 准备工作 (环境搭建)

在你的本地机器上（确保已安装 Node.js），执行以下命令创建项目：

```bash
# 创建 Vite + React 项目
npm create vite@latest love-counter -- --template react

# 进入目录
cd love-counter

# 安装必要依赖
npm install leancloud-storage lucide-react date-fns html2canvas jspdf
npm install -D tailwindcss postcss autoprefixer
npx tailwindcss init -p

```

### 2. 数据库配置 (LeanCloud 安全性)

在 LeanCloud 控制台，请务必进行以下操作：

1. **创建 Class**: 进入存储 -> 结构化数据，新建一个名为 `LoveRecord` 的 Class。
2. **权限设置**: 将 `LoveRecord` 的权限设置为“所有用户可读、可写”。（由于是静态私密应用，这样最简单；后期你可以通过设置一个固定的 `user_id` 来过滤数据）。

### 3. 核心代码完善 (`src/App.jsx`)

我们需要加入**实时自动刷新**和**更细腻的 Windows 11 UI 效果**。

```jsx
import React, { useState, useEffect } from 'react';
import AV from 'leancloud-storage';
import { format, eachDayOfInterval, startOfYear, endOfYear, isSameDay } from 'date-fns';
import { Heart, Download, Flame, Calendar as CalendarIcon, TrendingUp } from 'lucide-react';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';

// 初始化 LeanCloud
AV.init({
  appId: "fXnYOxrBJqYINPMyp49sG3Sh-gzGzoHsz",
  appKey: "4VGgTvlU2vWsdjK4QxM9ArcB",
  serverURL: "https://fxnyoxrb.lc-cn-n1-shared.com"
});

function App() {
  const [records, setRecords] = useState([]);
  const [isSyncing, setIsSyncing] = useState(false);

  // 1. 自动同步逻辑：每 30 秒拉取一次最新数据，确保两人看到的一致
  useEffect(() => {
    fetchData();
    const timer = setInterval(fetchData, 30000); 
    return () => clearInterval(timer);
  }, []);

  const fetchData = async () => {
    setIsSyncing(true);
    const query = new AV.Query('LoveRecord');
    query.descending('createdAt');
    query.limit(1000);
    try {
      const results = await query.find();
      setRecords(results.map(r => ({
        id: r.id,
        date: new Date(r.get('date')),
        rating: r.get('rating')
      })));
    } catch (err) {
      console.error("数据加载失败", err);
    }
    setIsSyncing(false);
  };

  // 2. 交互逻辑：点击日期或快速记录
  const addRecord = async (score) => {
    const LoveRecord = AV.Object.extend('LoveRecord');
    const record = new LoveRecord();
    record.set('date', new Date());
    record.set('rating', score);
    await record.save();
    fetchData();
  };

  // 3. 计算热力图颜色 (Win11 风格：浅粉到深紫红)
  const getCellColor = (date) => {
    const count = records.filter(r => isSameDay(r.date, date)).length;
    if (count === 0) return 'bg-white/30 border-white/20';
    if (count === 1) return 'bg-pink-200 border-pink-300';
    if (count === 2) return 'bg-pink-400 border-pink-500 shadow-[0_0_10px_rgba(236,72,153,0.3)]';
    return 'bg-gradient-to-br from-pink-600 to-purple-600 border-pink-700 shadow-lg';
  };

  return (
    <div className="min-h-screen bg-[#f3f3f3] bg-[url('https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2000')] bg-cover bg-fixed p-4 md:p-10">
      {/* 全局毛玻璃容器 */}
      <div id="capture-area" className="max-w-6xl mx-auto bg-white/60 backdrop-blur-2xl rounded-3xl shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] border border-white/40 overflow-hidden">
        
        {/* 顶部状态栏 */}
        <div className="px-8 py-6 flex justify-between items-center border-b border-white/20">
          <div>
            <h1 className="text-3xl font-black bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent flex items-center gap-3">
              <Heart className="fill-pink-500 text-pink-500" /> 爱计数
            </h1>
            <p className="text-xs text-slate-500 mt-1 uppercase tracking-widest font-bold">Record every sweet moment</p>
          </div>
          <div className="flex gap-3">
             <button onClick={fetchData} className={`p-2 rounded-full bg-white/50 hover:bg-white transition ${isSyncing ? 'animate-spin' : ''}`}>
               <TrendingUp size={20} className="text-slate-600"/>
             </button>
             <button onClick={() => {/* 导出函数 */}} className="px-5 py-2 bg-pink-600 text-white rounded-full font-bold text-sm shadow-lg shadow-pink-200 hover:bg-pink-700 transition active:scale-95">
               导出报告
             </button>
          </div>
        </div>

        <div className="flex flex-col md:flex-row">
          {/* 左侧：可视化看板 */}
          <div className="flex-1 p-8 space-y-8">
            <div className="grid grid-cols-3 gap-4">
              <StatCard label="累计次数" value={records.length} icon={<Flame className="text-orange-500"/>} color="bg-orange-50"/>
              <StatCard label="本周频率" value={calculateWeekly(records)} icon={<CalendarIcon className="text-blue-500"/>} color="bg-blue-50"/>
              <StatCard label="幸福指数" value={avgRating(records)} icon={<Star className="text-yellow-500"/>} color="bg-yellow-50"/>
            </div>

            {/* 年度热力图区域 */}
            <div className="bg-white/40 p-6 rounded-2xl border border-white/50">
               <h3 className="mb-4 font-bold text-slate-700">2024 甜蜜轨迹</h3>
               <div className="grid grid-flow-col grid-rows-7 gap-1 overflow-x-auto pb-4">
                 {/* 这里生成一整年的格子 */}
                 {generateYearGrid(records)}
               </div>
            </div>
          </div>

          {/* 右侧：快速交互面板 */}
          <div className="w-full md:w-80 bg-white/30 backdrop-blur-sm p-8 border-l border-white/20">
            <h3 className="font-bold mb-6 text-slate-700">新的一次？</h3>
            <div className="space-y-4">
              {[5, 4, 3, 2, 1].map(score => (
                <button 
                  key={score}
                  onClick={() => addRecord(score)}
                  className="w-full group flex items-center justify-between p-4 bg-white/50 hover:bg-pink-500 hover:text-white rounded-2xl border border-white transition-all duration-300"
                >
                  <span className="font-bold">{score} 星体验</span>
                  <Heart size={16} className="opacity-0 group-hover:opacity-100 transition-opacity fill-current"/>
                </button>
              ))}
            </div>
            <div className="mt-10 p-4 bg-pink-100/50 rounded-2xl border border-pink-200">
               <p className="text-xs text-pink-700 leading-relaxed italic">
                 "爱不仅是身体的契合，更是灵魂的共鸣。"
               </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

// 辅助组件：统计卡片
function StatCard({ label, value, icon, color }) {
  return (
    <div className={`${color} p-4 rounded-2xl border border-white shadow-sm`}>
      <div className="flex justify-between items-start mb-2">
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{label}</span>
        {icon}
      </div>
      <div className="text-2xl font-black text-slate-800">{value}</div>
    </div>
  );
}

// 辅助逻辑... (如 calculateWeekly, generateYearGrid 等)

```

---

### 4. 关键开发细节说明

#### ⚡️ 亚克力高级 UI (Tailwind 配置)

为了实现 Windows 11 的质感，在 `tailwind.config.js` 中建议加入自定义阴影：

```javascript
theme: {
  extend: {
    backdropBlur: {
      xs: '2px',
    },
    boxShadow: {
      'win11': '0 8px 32px 0 rgba(31, 38, 135, 0.07)',
    }
  }
}

```

#### 📊 热力图生成逻辑

在 GitHub Pages 上，横向排列的“年度热力图”最直观。使用 `date-fns` 的 `eachDayOfInterval` 配合 `grid-flow-col`，可以完美复现 GitHub 贡献图。

#### 📄 PDF 导出增强

为了让导出的 PDF 好看，你可以专门写一个隐藏的 `ReportTemplate` 组件，当点击导出按钮时：

1. 使用 `html2canvas` 渲染这个隐藏组件。
2. 报表内包含：**你们在一起的总天数**、**最频繁的一周**、**五星好评的占比饼图**。

#### 🔄 多设备同步流程

* **场景**：你在外面记录了一次，你女朋友在家打开电脑。
* **实现**：由于 `useEffect` 中设置了 `setInterval`，她不需要刷新页面，页面上的“累计次数”和“日历格子”会自动变红/加深。

---

### 5. 部署到 GitHub Pages (3分钟搞定)

1. **安装部署工具**: `npm install gh-pages --save-dev`
2. **修改 package.json**:
```json
"scripts": {
  "predeploy": "npm run build",
  "deploy": "gh-pages -d dist"
}

```


3. **运行发布**: `npm run deploy`
4. **最终地址**: `https://<你的github用户名>.github.io/love-counter/`

### 🌈 建议的下一步：

既然要 UI 好看， 你可以在应用顶部放一张你们的合照作为半透明背景。